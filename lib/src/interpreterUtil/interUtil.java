...
public class interUtil {
    ...
    public static String infixToPostfix(Stack<Token> infix) {
        ...
        for (Token token : infix) {
            if (isFloat(token.getLexeme()) || isInteger(token.getLexeme()) || isString(token.getLexeme()) || isBoolean(token.getLexeme())) {
    ...
    public static String evaluatePostfix(String postfix) {
        ...
        for (String token : tokens) {
            if (isArithmeticOp(token)) {
                ...
                String result = applyOperator(a, b, token);
                stack.push(result);
            } else {
    ...
    private static String applyOperator(String a, String b, String operator) {
        switch (operator) {
            case "plus":
